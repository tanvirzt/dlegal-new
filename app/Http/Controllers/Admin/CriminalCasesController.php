...
class CriminalCasesController extends Controller
{
    ...
    public function all_cases()
    {
        ...
        return view('litigation_management.cases.criminal_cases.all_cases', compact('user', 'group_name', 'client_category', 'client_name', 'next_date_reason', 'case_status', 'external_council', 'data', 'division', 'case_types', 'court', 'case_category', 'matter', 'case_cat'));
    }
    public function all_civil_cases()
    {    $user = User::all();
        $division = DB::table("setup_divisions")->get();
        $case_types = SetupCaseTypes::where('delete_status', 0)->get();
        $court = SetupCourt::where(['case_class_id' => 'Criminal', 'delete_status' => 0])->get();
    ...
    public function all_criminal_cases(){
...
//            ->orderBy('created_at','desc')
        ...
        $uploadedFileList = DB::table('criminal_cases_files')
        ->leftJoin('setup_documents_types', 'criminal_cases_files.documents_type_id',  'setup_documents_types.id')
        ->where('case_id', $id)
        ->get();          
